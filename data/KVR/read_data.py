    ...
    for d in dialogues:
        if(d['scenario']['task']['intent']=="navigate"): #"schedule" "navigate"
            ...
            for el in d['scenario']['kb']['items']:
                ...
                print("0 "+poi+" distance "+di[el['distance']]+" poi distance")
                print("0 "+poi+" traffic_info "+di[el['traffic_info']]+" poi traffic_info")
                print("0 "+poi+" poi_type "+di[el['poi_type']]+" poi poi_type")
                print("0 "+poi+" address "+di[el['address']]+" poi address")
                temp.append(di)
            temp += global_temp
            ...
            temp = []
            j = 1 
            print("0 today is "+d['scenario']['kb']['items'][0]["today"]+" today day")
            for el in d['scenario']['kb']['items']:
                ...
                for day in days:
                    print("0 "+loc+" "+day+"_weather "+ el[day].split(',')[0].rstrip().replace(" ", "_")+" location weather_attribute")
                    print("0 "+loc+" "+day+"_"+ el[day].split(',')[1].split(" ")[1] +" "+el[day].split(',')[1].split(" ")[3]+" location temperature")
                    print("0 "+loc+" "+day+"_"+ el[day].split(',')[2].split(" ")[1] +" "+el[day].split(',')[2].split(" ")[3]+" location temperature")
            temp += global_temp
                ...
                for el in d['scenario']['kb']['items']:
                    ...
                    for slot in slots:
                        if el[slot]=="-":
                            continue
                        if slot == "time":
                            print("0 "+ev+" "+slot+" "+el[slot].replace(" ", "")+" event "+slot)
                            di[el[slot]] = el[slot].replace(" ", "")
                        else:
                            print("0 "+ev+" "+slot+" "+el[slot].replace(" ", "_")+" event "+slot)
